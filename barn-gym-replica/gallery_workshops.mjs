...
const directoryPath = path.join(__dirname, 'public', 'images', 'workshops');
...
<html>
<head>
    <title>Downloaded Images Gallery (Workshops)</title>
    <style>
...
<body>
    <h2>Downloaded Images Gallery (Workshops)</h2>
    <div class="gallery">
        ${files.map(file => `
        <div class="item">
...
const galleryPath = path.join(__dirname, 'gallery_workshops.html');
...
(async () => {
    ...
    await page.screenshot({ path: 'gallery_workshops_full.png', fullPage: true });
    await browser.close();
    console.log('Gallery screenshot generated: gallery_workshops_full.png');
})();
