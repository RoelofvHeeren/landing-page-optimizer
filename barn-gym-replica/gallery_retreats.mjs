...
const directoryPath = path.join(__dirname, 'public', 'images', 'retreats');
...
<html>
<head>
    <title>Downloaded Images Gallery (Retreats)</title>
    <style>
...
<body>
    <h2>Downloaded Images Gallery (Retreats)</h2>
    <div class="gallery">
        ${files.map(file => `
        <div class="item">
...
const galleryPath = path.join(__dirname, 'gallery_retreats.html');
...
(async () => {
    ...
    await page.screenshot({ path: 'gallery_retreats_full.png', fullPage: true });
    await browser.close();
    console.log('Gallery screenshot generated: gallery_retreats_full.png');
})();
