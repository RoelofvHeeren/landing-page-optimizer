...
const images = fs.readdirSync('public/images/home').filter(f => !f.startsWith('.'));
...
<body>
    <h2>Downloaded Images Gallery</h2>
    <div class="grid">
        ${images.map(img => `
            <div class="card">
                <img src="${process.cwd()}/public/images/home/${img}" />
                <p>${img}</p>
            </div>
        `).join('')}
    </div>
...
(async () => {
    ...
    await page.screenshot({ path: 'gallery_full.png', fullPage: true });
    await browser.close();
    console.log('Gallery screenshot created.');
})();
